#!/usr/bin/python
# -*- coding: utf-8 -*-
# Code located at: https://github.com/juanrmn/Arduino-Telescope-Control/blob/master/main/python/telescope_server.py
...
def degStr_2_rad(d):
	...
	exp1 = re.compile(u'^-?[0-9]{,3}(º|ᵒ|°)[0-9]{,3}\'[0-9]{,3}([\']{2}|")$',re.UNICODE)  # Added by CP unicode flag 'u' and added the correct degree symbol
	exp2 = re.compile(u'^-?[0-9]{,3}\.[0-9]{,6}(º|ᵒ|°)$')						  # Added by CP unicode flag 'u' and added the correct degree symbol
	d = d.decode('utf-8') 	# added by CP

	if(not exp1.match(d) and not exp2.match(d)):
		logging.debug("Error parametro: %s" % d)
		return None
	elif(exp1.match(d)):
		#print "Paso ok por aqui"
		d = d.replace(u'°','.').replace("''",'.').replace("'",'.')				# Added by CP unicode flag 'u and added the correct degree symbol'
		d_dic = d.split('.')
		d_deg = float(d_dic[0])
		d_min = float(d_dic[1])
		d_sec = float(d_dic[2])
		...
		d_ndeg = float(d.replace(u'°',''))                              # Added by CP unicode flag 'u' the correct degree symbol
...
def eCoords2str(ra, dec, mtime):
	...
	return ('%dh%dm%00.0fs' % hour_min_sec(ra_h), '%d°%d\'%00.0f\'\'' % grad_min_sec(dec_d), strftime("%Hh%Mm%Ss", localtime(time_s)))
